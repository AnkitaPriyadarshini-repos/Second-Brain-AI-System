...
(function () {
  ...
  window.triggerPWAInstall = function () {
    ...
    if (deferredInstallPrompt) {
      ...
      deferredInstallPrompt.userChoice.then((choiceResult) => {
        if (choiceResult.outcome === 'accepted' && typeof window.showToast === 'function') window.showToast('Installing Juno AI…');
        deferredInstallPrompt = null;
      }).catch((err) => console.warn('Error during PWA installation prompt:', err));
    } else window.openPWAInstallInstructionsModal();
  };
  ...
  window.dismissPWABanner = function () {
    ...
    if (banner) { banner.classList.add('hiding'); setTimeout(() => { banner.style.display = 'none'; }, 300); }
    sessionStorage.setItem('juno_pwa_banner_dismissed', 'true');
  };

  window.showPWABanner = function () {
    const banner = document.getElementById('pwa-desktop-banner');
    if (banner && !isStandalone) { banner.style.display = 'flex'; banner.classList.remove('hiding'); }
  };

  window.checkPWAStatus = function () {
    return { isStandalone, canInstall: !!deferredInstallPrompt, serviceWorkerActive: 'serviceWorker' in navigator && !!navigator.serviceWorker.controller };
  };
  ...
  } else if (!isStandalone && sessionStorage.getItem('juno_pwa_banner_dismissed') !== 'true') window.showPWABanner();

  // Load the single production chat controller after legacy app bootstrap.
  // Version is bumped so stale PWA/browser caches cannot retain the old controller.
  setTimeout(() => {
    if (document.querySelector('script[data-juno-production-chat]')) return;
    const script = document.createElement('script');
    script.src = 'js/production-chat.js?v=3.0';
    script.dataset.junoProductionChat = 'true';
    script.onload = () => window.dispatchEvent(new CustomEvent('juno:production-chat-ready'));
    script.onerror = () => { if (typeof window.showToast === 'function') window.showToast('Juno chat controller failed to load. Refresh once to retry.', 'error'); };
