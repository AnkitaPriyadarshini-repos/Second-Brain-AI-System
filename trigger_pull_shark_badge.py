...
branch_name = f'feature/developer-telemetry-hud-{timestamp}'
...
# Add developer-hud.js on branch
with open('js/developer-hud.js', 'r', encoding='utf-8') as f:
    content_raw = f.read()

content_b64 = base64.b64encode(content_raw.encode('utf-8')).decode('utf-8')

file_payload = {
    'message': 'feat: Add developer telemetry & human engineering specs HUD module',
    'content': content_b64,
    'branch': branch_name
}
...
try:
    req_file = urllib.request.Request(
        f'https://api.github.com/repos/{repo}/contents/js/developer-hud.js',
        data=json.dumps(file_payload).encode(),
        headers=headers,
        method='PUT'
    )
    urllib.request.urlopen(req_file)
    print("Pushed js/developer-hud.js to branch.")
except Exception as e:
    print("File payload update note:", e)

# Create Pull Request
pr_payload = {
    'title': 'feat: Add developer telemetry & human engineering specs HUD module',
    'head': branch_name,
    'base': 'main',
    'body': 'Adds live vector latency telemetry (in ms), system architecture spec modals, keyboard shortcut hotkeys, and note vector inspection.'
}
